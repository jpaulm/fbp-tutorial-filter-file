...
When you are ready to add the filter function, you will of course need to delete the existing arrow - right click on the arrow itself, and select Delete; add the two additional blocks, and hook everything up! 
...
Now this is a procedural description of `ConcatStreams`, but you can also look at it from a "stream" point of view: from this point of view it is a sort of "gate", ensuring that no IPs of the stream arriving at any port `IN[i + 1]` can be received until *all* the IPs arriving at port `IN[i]` have been received (and presumably processed). 
